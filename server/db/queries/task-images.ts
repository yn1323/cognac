// タスク画像のCRUD操作

import type { TaskImage } from '@cognac/shared'
import type { CognacDb } from '../types.js'

/**
 * タスク画像のメタデータを保存する
 */
export function createTaskImage(
  db: CognacDb,
  data: {
    task_id: number
    file_path: string
    original_name: string
    mime_type: string
  },
): TaskImage {
  const stmt = db.prepare(`
    INSERT INTO task_images (task_id, file_path, original_name, mime_type)
    VALUES (@task_id, @file_path, @original_name, @mime_type)
  `)

  const result = stmt.run(data)
  // INSERT直後のデータは入力値+ROWIDから構築（不要なSELECTを回避）
  return {
    id: Number(result.lastInsertRowid),
    ...data,
    created_at: new Date().toISOString(),
  }
}

/**
 * タスク画像を1件取得する
 */
export function getTaskImage(db: CognacDb, id: number): TaskImage | undefined {
  const stmt = db.prepare(`SELECT * FROM task_images WHERE id = ?`)
  return stmt.get(id) as unknown as TaskImage | undefined
}

/**
 * タスクに紐づく画像一覧を取得する
 */
export function listTaskImages(db: CognacDb, taskId: number): TaskImage[] {
  const stmt = db.prepare(`SELECT * FROM task_images WHERE task_id = ? ORDER BY created_at ASC`)
  return stmt.all(taskId) as unknown as TaskImage[]
}

/**
 * タスク画像を削除する
 */
export function deleteTaskImage(db: CognacDb, id: number): boolean {
  const stmt = db.prepare('DELETE FROM task_images WHERE id = ?')
  return Number(stmt.run(id).changes) > 0
}
