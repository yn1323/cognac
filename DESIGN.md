...
```bash
pnpm add -D solitary-coding
pnpx solitary-coding init   # 設定ファイル・ディレクトリ生成
pnpx solitary-coding start  # ダッシュボード起動 + タスクランナー開始
```
...
│  callClaude() 共通ヘルパー関数                        │
│                                                     │
│  1. プロンプトを .solitary-coding/tmp/ に書き出し      │
...
async function callClaude(options: CallClaudeOptions): Promise<ClaudeResponse> {
  const tmpDir = '.solitary-coding/tmp'
...
```bash
cat .solitary-coding/tmp/prompt-execution.md | claude -p \
  --append-system-prompt-file .solitary-coding/tmp/system-execution.md \
  --max-turns 30 \
...
- タスクに添付された画像は `.solitary-coding/images/task-{id}/` に保存
- Phase 3の実行プロンプトに「`.solitary-coding/images/task-{id}/` のファイルを参照して」と記載
...
CREATE TABLE task_images (
  ...
  file_path TEXT NOT NULL,                     -- .solitary-coding/images/ 配下の相対パス
...
Solitary Coding自体をSolitary Codingで開発できる。
...
残りの機能開発をSolitary Coding自身のタスクキューに投入して自己開発に切り替える。
...
- タスクランナーのプロセスを再起動する変更（サーバー設定変更等）は、
  マージ後に手動で `pnpx solitary-coding start` を再実行する必要がある。
