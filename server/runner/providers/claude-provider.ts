...
import { cleanupTmpFiles, setupAbortHandler, setupProcess, writeTmpFiles } from './process-utils.js'
import type {
    ...
    try {
      return await new Promise<CliResponse>((resolve, reject) => {
        ...
        child.on('close', (code: number | null) => {
          ...
          console.log(
            `[ClaudeProvider] プロセス終了 code=${code} lines=${lineCount} result=${result.length}文字 duration=${durationMs}ms`,
          )
    ...
    try {
      return await new Promise<CliResponse>((resolve, reject) => {
        ...
        child.on('close', (code: number | null) => {
          ...
          console.log(
            `[ClaudeProvider] プロセス終了 code=${code} stdout=${totalBytes}bytes duration=${durationMs}ms`,
          )
          if (stderr) console.log(`[ClaudeProvider] stderr:\n${stderr}`)
