...
function CommandCard({
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className={`console-command-card flex w-full flex-col gap-2 rounded-lg border bg-[#fafafa] p-3 text-left ${
        selected ? `is-selected ${cfg.borderColorClass}` : 'border-[#e5e5e5]'
      }`}
    >
      {/* Top: name + actions */}
