 ...
 * 作成日時の古い順（時系列）で返す
 */
export function getLogsByTaskId(db: CognacDb, taskId: number): ExecutionLog[] {
 ...
 * 実行ログを1件取得する
 */
export function getLog(db: CognacDb, id: number): ExecutionLog | undefined {
 ...
 */
export function deleteLogsByTaskId(db: CognacDb, taskId: number): number {
